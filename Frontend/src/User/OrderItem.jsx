  ...
  return (
    <div style={{ background: 'linear-gradient(to right, #4f46e5, #8b5cf6, #ec4899)', minHeight: '100vh', padding: '20px 0' }}>
      ...
      <div style={{ display: 'flex' }}>
        <div className="max-w-md mx-auto mt-8 p-4 border rounded shadow-lg bg-white" style={{ borderRadius: '12px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
          <h2 className="text-2xl font-semibold" style={{ color: '#4f46e5' }}>Your order is almost Done!</h2>
          <form onSubmit={handleSubmit}>
            <div>
              <label className="block text-gray-600 text-center" style={{ paddingTop: "10px" }}>Address:</label>
              <div className="input-container">
                <input type="text" id="myInput" className="w-48 p-2 border border-gray-300 rounded focus:outline-none" placeholder=" " style={{ width: "340px" }}
                  name="flatno"
                  value={formData.flatno}
                  onChange={handleChange}
                />
                <label htmlFor="myInput" className="absolute left-2 transform -translate-y-1/2 bg-white px-1 pointer-events-none transition-transform">
                  Flat no
                </label>
              </div>
            </div><br />
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div>
                <div className="input-container">
                  <input type="text" id="myInput" className="w-48 p-2 border border-gray-300 rounded focus:outline-none" placeholder=" "
                    style={{ width: "140px" }}
                    name="city"
                    value={formData.city}
                    onChange={handleChange}
